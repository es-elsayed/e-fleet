<script setup>

import { ref, onMounted, onDeactivated } from 'vue';

const showScrollButton = ref(false);

onMounted(() => window.addEventListener('scroll', handleScroll));
onDeactivated(() => window.removeEventListener('scroll', handleScroll));


const handleScroll = (lang) => {
    showScrollButton.value = window.scrollY > 300;
}

const scrollUp = (lang) => {
    window.scrollTo({
        top: 0,
        behavior: 'smooth'
    });
}

</script>
<template>
    <a href="javascript:void(0);" @click="scrollUp" v-show="showScrollButton"
    class="fixed z-50 rounded-full shadow bottom-16 rtl:right-8 ltr:left-8"
        >
        <svg xmlns="http://www.w3.org/2000/svg" class="fill-pr-500" height="3em" viewBox="0 0 512 512">
            <path
                d="M256 512A256 256 0 1 0 256 0a256 256 0 1 0 0 512zM135.1 217.4l107.1-99.9c3.8-3.5 8.7-5.5 13.8-5.5s10.1 2 13.8 5.5l107.1 99.9c4.5 4.2 7.1 10.1 7.1 16.3c0 12.3-10 22.3-22.3 22.3H304v96c0 17.7-14.3 32-32 32H240c-17.7 0-32-14.3-32-32V256H150.3C138 256 128 246 128 233.7c0-6.2 2.6-12.1 7.1-16.3z" />
        </svg>
    </a>
</template>
